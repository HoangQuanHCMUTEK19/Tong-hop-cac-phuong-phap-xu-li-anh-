...
L = 256
#Làm xói mòn ảnh (hình ảnh nhị phân)
...
#Chuyển phần trắng thành đen và có viền trắng 
def Boundary(imgin):
    w = cv2.getStructuringElement(cv2.MORPH_RECT,(3,3))
    temp = cv2.erode(imgin,w)
    imgout = imgin - temp
    return imgout
#Lấp lỗ đen thành nền trắng 
def HoleFill(imgin):
    imgout = imgin
    M, N = imgout.shape
...
#chuyền các phần nền ngoài của các phần tử màu trắng thành đen => là đếm các phần tử được làm nổi 
def CountRice(imgin):
